...
Focusing on cluster 8, which seems to be made of platelets:

```{r heatmap8, fig.wide=TRUE}
current <- marker.out[["8"]]
cur.exprs <- logcounts(sce)[rownames(current)[current$Top <= 10],]
...
... and 6, which seems to contain damaged cells with high mitochondrial content:

```{r heatmap6, fig.wide=TRUE}
current <- marker.out[["6"]]
cur.exprs <- logcounts(sce)[rownames(current)[current$Top <= 10],]
...
... and 14, the only defining characteristic of which is the complete loss of a variety of proteins.
This probably represents damaged cells that have lost cytoplasmic transcripts, and are in fact so damaged that even the mitochondria have been lost.

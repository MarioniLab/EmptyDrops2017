...
In summary, all barcodes with $t_b$ above the inflection point are retained, regardless of $P_b$.
We stress that this approach is different from existing methods, due to the addition of our testing procedure.
...
Note that we only perform the correction on the $p$-values for barcodes that have $t_b$ greater than $T$ and less than the inflection point.
This reduces the severity of the correction given that barcodes with $t_b$ outside of this range have fixed outcomes, i.e., barcodes with $t_b < T$ will always be discarded while barcodes with $t_b$ above the inflection point will always be retained.
